...
static void evalR0_0fs4d(REAL *pts, unsigned dim, REAL *p, const REAL *c,
	  ...
	  for (iR = 0; iR < nR; ++iR) {
		  ...
		  ... not completely clear to me why we don't just use
	       ...
	       if (result_asc != 0 && err != 0) {
		    REAL scale = pow((200 * err / result_asc), 1.5Q);
     ...
     while (numEval < maxEval || !maxEval) {
	  ...
	  if (parallel) {
	       ...
	       do {
		    ...
		    nR += 2;
	       } while (regions.n > 0 && (nR < 10 || nR + 2 <= nR_alloc) && (numEval < maxEval || !maxEval)); //Feng
	       if (eval_regions(nR, R, f, fdata, r)
		   || heap_push_many(&regions, nR, R))
		    goto bad;
          
          if(minEval > 10 && PrintHooker != NULL && numEval - runs > minEval) {
              /* copied from below */
...
static int cubature(unsigned fdim, integrand_v f, void *fdata, 
     ...
     status = !h.data ? FAILURE
	  : rulecubature(r, fdim, f, fdata, &h,
				minEval, maxEval, reqAbsError, reqRelError, norm,
				val, err, parallel, PrintHooker);
     destroy_hypercube(&h);
     destroy_rule(r);
     return status;
}
...
int hcubature_v(unsigned fdim, integrand_v f, void *fdata, 
     ...
     return cubature(fdim, f, fdata, dim, xmin, xmax, 
		     minEval, maxEval, reqAbsError, reqRelError, norm, val, err, 1, PrintHooker);
}

//#include "vwrapper.h"
/* vectorized wrapper around non-vectorized integrands */
typedef struct fv_data_s { integrand f; void *fdata; } fv_data;
static int fv(unsigned ndim, long long npt,
          const REAL *x, void *d_,
...
int hcubature(unsigned fdim, integrand f, void *fdata, 
     ...
     ret = cubature(fdim, fv, &d, dim, xmin, xmax, 
		    0, maxEval, reqAbsError, reqRelError, norm, val, err, 0, NULL);
     return ret;
}
