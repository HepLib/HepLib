...
public:
	...
	~Workspace();
	int qag(FtnBase & f, Real a, Real b, Real epsabs, Real epsrel, Real& result, Real& abserr, PrintHookerType PrintHooker=NULL, bool parallel=false);
...
void Workspace::append_interval (Real a1, Real b1, Real area1, Real error1) {
	alist[size] = a1;
	blist[size] = b1;
	rlist[size] = area1;
	elist[size] = error1;
...
void Workspace::initialise (Real a, Real b) {
	size = 0;
	nrmax = 0;
	i_work = 0;
	alist[0] = a;
...
void Workspace::set_initial_result (Real result, Real error) {
	size = 1;
	rlist[0] = result;
	elist[0] = error;
}
...
int Workspace::subinterval_too_small (Real a1, Real a2, Real b2) {
	const Real e = mpfr::machine_epsilon();
	const Real u = mpfr::minval();

	Real tmp = (1 + 100 * e) * (abs(a2) + 1000 * u);
	int status = (abs(a1) <= tmp && abs(b2) <= tmp);
	return status;
}

void Workspace::update (Real a1, Real b1, Real area1, Real error1,
								 Real a2, Real b2, Real area2, Real error2) {
...
/* -------------------------------------------------------------------------- */
int Workspace::qag(FtnBase& f, Real a, Real b,
							Real epsabs, Real epsrel, Real& result, Real& abserr, PrintHookerType PrintHooker, bool parallel)
{
	Real area, errsum;
	...
	if (epsabs <= Real(0) && epsrel < 50 * mpfr::machine_epsilon()) {
      static const char*
	...
	this->qk(f, a, b, result0, abserr0, resabs0, resasc0, parallel);
	 ...
	 */
	round_off = 50 * mpfr::machine_epsilon() * resabs0;
...
if(PrintHooker) {
	...
	do
	{
      ...
      this->qk(f, a1, b1, area1, error1, resabs1, resasc1, parallel);
      this->qk(f, a2, b2, area2, error2, resabs2, resasc2, parallel);
      ...
      if (resasc1 != error1 && resasc2 != error2) {
			...
			if (abs(delta) <= 1.0e-5 * abs(area12)
				 && error12 >= 0.99 * e_i)
			{
				roundoff_type1++;
			}
