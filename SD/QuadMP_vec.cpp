/**
 * @file
 * @brief Numerical Integrator using QuadMP_vec
 */
...
ex QuadMP_vec::mp2ex(const mpREAL & num) {
    ostringstream oss;
...
int QuadMP_vec::Wrapper(unsigned ydim, mpREAL *y, mpREAL *e, unsigned xdim, const mpREAL *x, void *fdata) {
    auto self = (QuadMP_vec*)fdata;
...
void QuadMP_vec::DefaultPrintHooker(mpREAL* result, mpREAL* epsabs, size_t * nrun, void *fdata) {
    auto self = (QuadMP_vec*)fdata;
...
ex QuadMP_vec::Integrate(size_t n) {
    ...
    if(nok) {
        ...
        if( (abs_res < mpfr_eps) && (abs_est < mpfr_eps) ) {
            cout << ErrColor << "QuadMP_vec Failed with 0 result returned!" << RESET << endl;
            return NaN;
        }
    }
