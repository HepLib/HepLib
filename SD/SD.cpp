...
cout << "vec_map3.size = " << vec_map3.size() << endl;
    ...
    void SecDec::Scalelesses(bool verb) {
        ...
        auto sl_res =
        GiNaC_Parallel(ParallelProcess, FunExp.size(), 0, [&](int idx)->ex {
    ...
    void SecDec::SDPrepares() {
        ...
        while(ibp_in_vec.size()>0) {
            ...
            spn << "IBP-" << (pn-1);
            auto ibp_res =
            GiNaC_Parallel(ParallelProcess, ibp_in_vec.size(), 0, [&](int idx)->ex {
                // return lst
        ...
        auto res =
        GiNaC_Parallel(ParallelProcess, ibp_res_vec.size(), 0, [&](int idx)->ex {
        ...
        if(zResides) {
            Integrands =
            GiNaC_Parallel(ParallelProcess, ints.size(), 0, [&](int idx)->ex {
                auto item = ints[idx];
                ex it = item;
                if(it.has(vz)) {
                    exset cts;
    ...
    void SecDec::EpsEpExpands() {
        ...
        auto res =
        GiNaC_Parallel(ParallelProcess, Integrands.size(), 0, [&](int idx)->ex {
