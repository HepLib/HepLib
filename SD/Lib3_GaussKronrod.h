...
namespace {
    ...
    public:
        ...
        void qk(FtnBase& f, Real a, Real b,
    ...
    Real
    GaussKronrod::chebyshev_series_deriv(Real x)
    {
        Real d1(0), d2(0);
        Real y2 = 2 * x; // linear term for Clenshaw recursion
    ...
    Real GaussKronrod::rescale_error (Real err, const Real result_abs,
                                                         const Real result_asc)
    {
        err = abs(err);
    ...
    //      Real fval2 = f.function( center + abscissa , f.params);
        ...
        for (j = 0; j < n_ - 1; j++)
        {
          result_asc += wgk_[j] * (abs(fv1[j] - mean) +
                                             abs(fv2[j] - mean));
        }
