...
void HCubature::DefaultPrintHooker(qREAL* result, qREAL* epsabs, long long int* nrun, void *fdata) {
    ...
    if((isnan(result[0]) || isnan(result[1]) || isnan(epsabs[0]) || isnan(epsabs[1])) || (isinf(result[0]) || isinf(result[1]) || isinf(epsabs[0]) || isinf(epsabs[1]))) {
        ...
        *nrun = self->MaxPTS + 1979;
        ...
        if(Verbose>10 && self->RunMAX>0) cout << ErrColor << "     Exit with NaN, LastN=" << self->lastNRUN << RESET << endl;
        return;
    }
    ...
    if(self->RunMAX>0 && (epsabs[0] > 1E30*self->EpsAbs || epsabs[1] > 1E30*self->EpsAbs)) {
        ...
        *nrun = self->MaxPTS + 1979;
        if(self->LastState>0) self->LastState = -1;
        if(Verbose>10 && self->RunMAX>0) cout << WarnColor << "     Exit with EpsAbs, LastN=" << self->lastNRUN << RESET << endl;
        return;
    }
