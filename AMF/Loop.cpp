...
namespace HepLib {
    ...
    void Loop::InitDE() {
        ...
        for(int i=0; i<iPropagator.nops(); i++) {
            ...
            if(!sq) {
                // make sure the i-th index for all MIs are 0
                for(auto mi : MIntegral) {
                    if(!mi.op(1).op(i).is_zero()) {
                        cout << "MI: " << MIntegral << endl;
                        throw Error("the index is NOT zero for Non-squared propagator!");
                    }
                }
                if(pi.match(w1*w2)) {
        ...
        if(true) {
            ...
            Combinations(vLoopPat.nops(), nL, [&](const int* is) {
                ...
                if(sols.nops()>0) {
                    auto xprop = subs(momProp, sols).subs(loops_back);
                    for(auto li : Internal) {
                        int nn = 0;
                        for(auto xi : xprop) if(xi.has(li)) nn++;
                        if(nn<nmin) {
                            nmin = nn;
                            min_prop = subs(iPropagator, sols).subs(loops_back);
                            eLoop = li;
                        }
                    }
                }
            });
            iPropagator = ex_to<lst>(min_prop);
        }
    ...
    void Loop::Poles() {
        ...
        } else throw Error("Loop::InitDE, NO root found.");
    ...
    lst Loop::apart(const ex & lps, const ex & eps, const lst & expr) {
        int nl = lps.nops();
        int ne = eps.nops();
    ...
    ex Loop::region(const lst & ls, const lst & es, const lst & ps, const lst & ns) {
    ...
    matrix Loop::oo() {
        if(!In_GiNaC_Parallel && Verbose>0) cout << pre << "\\--" << WHITE << "Loop Mode @ infinity ..." << RESET << endl;
...
//                                    cc = series_ex(pow(p+1, -mi.op(1).op(prop_idx)), p, xon);
                                ...
                                if(xon!=0) throw Error("Loop::oo, xon=!0 NOT supported Yet!");
                                lst lps, sps, lns, sns;
                                auto props = lc.op(2);
                                auto pows = lc.op(3);
                                auto ns = mi.op(1);
                                    ...
                                    if(!e.has(F(w1,w2))) return e;
                                        ...
                                        for(auto item : e.op(1)) {
                                            if(item!=1 && !item.is_zero()) {
                                                cout << "Vacuum: " << e << endl;
                                                throw Error("Loop::oo, Unknow Vacuum Integral Found.");
                                            }
                                            sum += ex2int(item);
                                        }
    ...
    void Loop::oo2o(matrix & iBC) {
        int N = Mat.rows();
        int npts = pts.nops();
        if(!In_GiNaC_Parallel && Verbose>0) cout << pre << "\\--" << WHITE << "Loop Mode @ middle " << npts-1 << " points ... " << RESET << endl;
        ...
        for(int i=npts-2; i>=0; i--) {
            if(!In_GiNaC_Parallel && Verbose>0 && Verbose<=5) {
                cout << "\r                                          \r" << flush;
                cout << pre << "\\--" << "Loop Mode @ middle points: " << npts-1 << "|" << npts-1-i << flush;
            }
            auto x1 = pts.op(i);
            auto x2 = pts.op(i+1);
            auto dx = x1-x2;
    ...
    matrix Loop::o() {
        if(!In_GiNaC_Parallel && Verbose>0) cout << pre << "\\--" << WHITE << "Loop Mode @ origin ..." << RESET << endl;
    ...
    void Loop::operator()() {
        if(!In_GiNaC_Parallel && Verbose>0) cout << WHITE << pre << "\\--Loop Mode Start @ " << now() << RESET << endl;
        ...
        for(int i=0; i<n; i++) {
            ...
            if(e.has(F(w1,w2))) {
                ...
                throw Error("Loop Mode: F still exists!");
            }
            NIntegral.append(e);
        }
                        
        if(!In_GiNaC_Parallel && Verbose>0) cout << WHITE << pre << "\\--Loop Mode Done @ " << now() << RESET << endl;
    }
        
    ex Loop::Vacuum(int nl, int np) { // single mass
    ...
    bool Loop::isVacuum(int nl, int np) { // single mass
