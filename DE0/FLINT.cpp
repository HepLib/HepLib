...
namespace HepLib {
    ...
    bool error_pass(arb_t r, bool prt) {
        auto dp = error_pass_dp;
        auto fp = dp2fp(dp);
        if(get_rel_err(r)>-dp) {
            mag_t mag;
            arb_get_mag(mag,r);
            if(mag_cmp_2exp_si(mag,-fp)<0) { mag_clear(mag); return true; }
            if(prt) { cout << endl; arb_printd(r,30); cout << endl; }
            mag_clear(mag); return false;
        }
        return true;
    }
    
    bool error_pass(acb_t z, bool prt) {
        arb_t ri;
        arb_init(ri);
        acb_get_real(ri,z);
        bool pass = true;
        if(!error_pass(ri,prt)) pass = false;
        else {
            acb_get_imag(ri,z);
            pass = error_pass(ri,prt);
        }
        arb_clear(ri);
        return pass;
    }
    
    bool error_pass(arb_mat_t m, bool prt) {
        auto nr = arb_mat_nrows(m);
        auto nc = arb_mat_ncols(m);
        for(int r=0; r<nr; r++) for(int c=0; c<nc; c++) {
            if(!error_pass(arb_mat_entry(m,r,c),prt)) return false;
        }
        return true;
    }
    
    bool error_pass(acb_mat_t m, bool prt) {
        auto nr = arb_mat_nrows(m);
        auto nc = arb_mat_ncols(m);
        for(int r=0; r<nr; r++) for(int c=0; c<nc; c++) {
            if(!error_pass(arb_mat_entry(m,r,c),prt)) return false;
    ...
    void to_acb(acb_mat_t m, const matrix & mat, slong dp) {
        ...
        auto nc = mat.cols();
        for(int r=0; r<nr; r++) for(int c=0; c<nc; c++) {
            to_acb(acb_mat_entry(m,r,c), mat(r,c), dp);
        }
    }
    ...
    void MX::lcm() { // Q is LCM of denominators in x*M, M is now x*M
        fmpz_poly_t x;
        fmpz_poly_init(x);
