...
namespace EoD {
    ...
    public:
        ...
        NDEH(const NDEH & b);
        NDEH(const symbol & x);
        NDEH(const matrix & m, const symbol & x);
        NDEH(const symbol & x, const matrix & m);
        ...
        void Fuchsify();
        void Shear();
        void Shear0(); // version 0
        
        exvector las;
        CMatrix Series(const int xN, const lst & las={}); 
        matrix Series(const ex & x0, const int xN, const lst & las={}); // C matrix
    ...
    //private:
        ...
        matrix RU(matrix C, const ex & iet1, const ex & iet2, NDEH & de);
    };
